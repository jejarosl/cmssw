#include <cmath>
#include <string>
#include "RecoHGCal/TICL/plugins/LinkingAlgoByMLP.h"
...
std::vector<float> calculate_dist_mlp(const std::vector<std::vector<float>>& lcs_i, const std::vector<std::vector<float>>& lcs_j) {
...
{ 
    ...
    for (auto x : mp){ 
        ...
        std::cout << x.first << ":" << x.second / float(N-1) << std::endl;
    }*/
    return degrees;
}
...
void Graph_mlp::addEdge(int v, int w) {
  adj[v].push_back(w);  // Add w to v’s list.
}

void Graph_mlp::DFSUtil(int v) {
...
LinkingAlgoByMLP::LinkingAlgoByMLP(const edm::ParameterSet &conf)
...
LinkingAlgoByMLP::~LinkingAlgoByMLP() {}

void LinkingAlgoByMLP::initialize(const HGCalDDDConstants *hgcons,
...
void LinkingAlgoByMLP::linkTracksters(const edm::Handle<std::vector<reco::Track>> tkH,
  ...
  std::cout << "Linking Algo by MLP" << std::endl;
  // MLP input names
  ...
  for (unsigned i = 0; i < N; ++i) {
    ...
    // 27 - will be added later in code
    // 28 - will be added later in code
    
    features.push_back(ts.time());              // 29: time
  ...
  for (int i = 0; i < N; i++){
    ...
    for (auto & i_neighbour : ticlGraph.getNode(i).getInner()){
      ...
      // Calculate distances
      auto dists = calculate_dist_mlp(lcs_i, lcs_j);
  ...
  for (int i = 0; i < N; i++){
      ...
      if (DEBUG == 1){
        std::cout << "node degree inner: " << node_degrees[i]/node_degree_max << std::endl;
        std::cout << "node degree centr: " << degrees_centr[i] << std::endl;
      }
  
     features.insert(features.begin() + 27 * (i+1), node_degrees[i]/node_degree_max); // 27 - inner node degrees
     features.insert(features.begin() + 28 * (i+1), degrees_centr[i]); // 28 - degreer centrality
  }
  ...
  // Create a graph
  Graph_mlp g;
  const auto classification_threshold = 0.99999;
...
void LinkingAlgoByMLP::fillPSetDescription(edm::ParameterSetDescription &desc) {
